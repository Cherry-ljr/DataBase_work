...
namespace 书店管理系统
{
    public partial class Manager : Form
    {
        ...
        private void Cus_Delete_Click(object sender, EventArgs e)
        {
            ...
            conn.Open();
            SqlDataAdapter daAuthors = new SqlDataAdapter("select nickname'顾客昵称',username'用户名',passwd'密码',vip'会员',tel '电话',address '地址',cus_sex'性别',cus_age'年龄' from [Customer]", conn);
        ...
        private void Cus_Search_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection())
                {
                    ...
                    String sql = " select nickname'顾客昵称',username'用户名',passwd'密码',vip,tel'电话',address '地址',cus_sex'性别',cus_age'年龄' from [Customer] where nickname like '%"
                        + cus_name.Text.Trim()
                        + "%'" + "or username like '%" + cus_name.Text.Trim() + "%'";//这个表名一定要加上[]; // 查询语句
        ...
        private void Search_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection())
                {
                    ...
                    String sql = "select Order_no '订单编号',username'用户名',Book_no'订购书籍编号',Book_num'订购数量',status'是否处理',time'订单时间' from [OrderInfo] ";// 查询语句
        ...
        private void OK_Click(object sender, EventArgs e)
        {
            ...
            SqlConnection conn = new SqlConnection("Data Source=PC-20170513IEDS\\SQLEXPRESS;Initial Catalog=BookStore;Integrated Security=True");
            conn.Open();
            SqlDataAdapter daAuthors = new SqlDataAdapter("select Order_no '订单编号',username'用户名',Book_no'订购书籍编号'Book_num'订购数量',status'是否处理',time'订单时间' from [OrderInfo]", conn);
        ...
        private void Book_Search_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection())
                {
                    ...
                    String sql = " select Book_no'书籍编号',name'书籍名称',author'作者',publish'出版社',origin_price '原价',price'售价',Sup_no'供应商编号',Book_category'书籍类别' from [Book] "; // 查询语句
        ...
        private void Book_Delete_Click(object sender, EventArgs e)
        {
            ...
            SqlConnection conn = new SqlConnection("Data Source=PC-20170513IEDS\\SQLEXPRESS;Initial Catalog=BookStore;Integrated Security=True");
            conn.Open();
            SqlDataAdapter daAuthors = new SqlDataAdapter(" select Book_no'书籍编号',name'书籍名称',author'作者',publish'出版社',origin_price '原价',price'售价',Sup_no'供应商编号',Book_category'书籍类别' from [Book]", conn);
            DataSet dsPubs = new DataSet("Pubs12");
