    ...
    public partial class Login : Form
    {
        ...
        private void Login_Btn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (status == 0)
                {
                    SqlCommand cmd = new SqlCommand("select * from [Manager] where username='" + username.Text.Trim() + "' and passwd='" + password.Text.Trim() + "'", conn);//这个表名一定要加上[]
                ...
                {
                    SqlCommand cmd = new SqlCommand("select * from [Customer] where username='" + username.Text.Trim() + "' and passwd='" + password.Text.Trim() + "'", conn);//这个表名一定要加上[]
                    SqlDataReader sdr = cmd.ExecuteReader();
                    sdr.Read();
                    if (sdr.HasRows)//登录成功
                    {
